...
pdf(title, width=8.27/2, height=11.69/5, useDingbats=FALSE)
	ggplot(m, aes(x=factor(dimer, levels=r), y=mean.milliDiv, group=interaction(dimer, age), fill=age)) + geom_boxplot(notch=TRUE, lwd=0.25, outlier.size=0.5) + theme_minimal(base_size=8) + scale_fill_manual(values=c("red", "blue"), labels=c("species-specific", "overlapping"), name="CircRNA loci") + theme(plot.title=element_text(hjust=0, vjust=1, face="bold", size=10), legend.key.size = unit(0.5, "cm"), axis.text.x=element_text(angle=30)) + xlab("top-5 dimers") + ylab("milliDiv") + ylim(0, ymax*1.15) + ggtitle(paste("MilliDivs", species, sep=", ")) + geom_text(data=pvalues, aes(x=factor(dimer, levels=r), y=ymax, label=paste("p = ", p, sep=""), group=NULL, fill=NULL, fontface="italic"), size=2.4)
dev.off()
