...
# read data
dimers <- read.table(paste("~/", species, "/repeats/dimers_", species, ".txt", sep=""), sep="\t", as.is=TRUE)
df <- read.table(paste("~/", species, "/repeats/", species, "_distmx.txt", sep=""), sep="\t", as.is=TRUE, row.names=1)
names <- read.table(paste("~/", species, "/repeats/repeat_names.txt", sep=""), sep="\t", as.is=TRUE)

# frequency analysis
nb <- table(dimers$V3)[1]
nf <- table(dimers$V3)[2]
dimers.fl <- subset(dimers, V3 == "flanking")
dimers.fl <- dimers.fl[order(dimers.fl$V2, decreasing=TRUE),]
top5 <- subset(dimers.fl, V2 != "Inf")$V1[1:5]
...
cofold <- read.table("~/human/repeats/RNAcofold_pairwise_human.txt", sep="\t", as.is=TRUE, header=TRUE)
...
# summary(pc) used for cumulative explained variance of PC1 and PC2
...
if (species == "opossum") {
	...
	m$rep1[m$rep1 == "MAR1a_Mdo"] <- "Mar1a_Mdo"
	m$rep2[m$rep2 == "MAR1a_Mdo"] <- "Mar1a_Mdo"
	m$rep1[m$rep1 == "MAR1b_Mdo"] <- "Mar1b_Mdo"
	m$rep2[m$rep2 == "MAR1b_Mdo"] <- "Mar1b_Mdo"
}
...
pdf(paste(species, "_repeatPhylogeny.pdf", sep=""), width=8.27*0.6, height=11.69/4, useDingbats=FALSE)
	ggplot(d2) + theme_minimal(base_size=8) + geom_line(data=subset(lines, selfloop == 0), aes(x=x, y=y, colour=rev(rank)), lwd=1.5) + geom_point(data=subset(lines, selfloop == 1), aes(x=x-(abs(y)*0.02), y=y+(abs(y)*0.2), colour=rev(rank)), shape=1, size=4, stroke=2, show.legend=FALSE) + geom_point(aes(x=PC1, y=PC2, fill=V2, size=freq), shape=21) + guides(fill=guide_legend(ncol = 2, title.position="top", title="Repeat class", size=2), colour=guide_legend(ncol = 2, title.position="top", title="Top5 rank", override.aes=list(size = 1)), size=guide_legend(ncol = 2, title.position="top", title="Frequency")) + scale_colour_manual(values=c('#c6dbef','#9ecae1','#6baed6','#4292c6','#2171b5'), labels=5:1) + xlab("PC1") + ylab("PC2") + scale_fill_manual(values=c('#f768a1', '#7a0177', '#feebe2')) + geom_text(data=unique(lines[,c(1:3)]), aes(x=x+0.05, y=y+0.05, label=repeatname), fontface="bold", size=2.8) + theme(plot.title=element_text(hjust=0, vjust=1, face="bold", size=10), legend.key.size = unit(0.5, "cm")) + ggtitle(paste("A: ", species, " repeat dimers", sep="")) 
dev.off()
...
dev.off()
	...
	# full plot
pdf(paste(species, "_repeatPhylogeny.pdf", sep=""), width=8.27*0.6, height=11.69/4, useDingbats=FALSE)
	ggplot(d2) + theme_minimal(base_size=8) + geom_line(data=subset(lines, selfloop == 0), aes(x=x, y=y, colour=rev(rank)), lwd=1.5) + geom_point(data=subset(lines, selfloop == 1), aes(x=x-(abs(y)*0.007), y=y+(abs(y)*0.1), colour=rev(rank)), shape=1, size=4, stroke=2, show.legend=FALSE) + geom_point(aes(x=PC1, y=PC2, fill=V2, size=freq), shape=21) + guides(fill=guide_legend(ncol = 2, title.position="top", title="Repeat class", size=2), colour=guide_legend(ncol = 2, title.position="top", title="Top5 rank", override.aes=list(size = 1)), size=guide_legend(ncol = 2, title.position="top", title="Frequency")) + scale_colour_manual(values=c('#c6dbef','#9ecae1','#6baed6','#4292c6','#2171b5'), labels=5:1) + xlab("PC1") + ylab("PC2") + scale_fill_manual(values=c('#feebe2','#7a0177')) + geom_text(data=unique(lines[,c(1:3)]), aes(x=x+0.06, y=y, label=repeatname), fontface="bold", size=2.8) + theme(plot.title=element_text(hjust=0, vjust=1, face="bold", size=10), legend.key.size = unit(0.5, "cm")) + ggtitle(paste("C: ", species, " repeat dimers", sep=""))
dev.off()
...
pdf(paste(species, "_repeatPhylogeny.pdf", sep=""), width=8.27*0.6, height=11.69/4, useDingbats=FALSE)
	ggplot(d2) + theme_minimal(base_size=8) + geom_line(data=subset(lines, selfloop == 0), aes(x=x, y=y, colour=rev(rank)), lwd=1.5) + geom_point(data=subset(lines, selfloop == 1), aes(x=x+(abs(x)*0.007), y=y+(abs(y)*0.004), colour=rev(rank)), shape=1, size=4, stroke=2) + geom_point(aes(x=PC1, y=PC2, fill=V2, size=freq), shape=21) + guides(fill=guide_legend(ncol = 2, title.position="top", title="Repeat class", size=2), colour=guide_legend(ncol = 2, title.position="top", title="Top5 rank", override.aes=list(size = 1)), size=guide_legend(ncol = 2, title.position="top", title="Frequency")) + scale_colour_manual(values=c('#c6dbef','#9ecae1','#6baed6','#4292c6','#2171b5'), labels=5:1) + xlab("PC1") + ylab("PC2") + scale_fill_manual(values=c('#feebe2','#7a0177')) + geom_text(data=unique(lines[,c(1:3)]), aes(x=x, y=y+0.075, label=repeatname), fontface="bold", size=2.8) + theme(plot.title=element_text(hjust=0, vjust=1, face="bold", size=10), legend.key.size=unit(0.5, "cm")) + ggtitle(paste("D: ", species, " repeat dimers", sep=""))
dev.off()
}