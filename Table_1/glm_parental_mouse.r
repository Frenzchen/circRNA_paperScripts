...
# 1. Normalisation
df <- read.table("geneTable_mouse_glm.txt", sep="\t", header=TRUE, as.is=TRUE)
...
glm.pred.coef$species <- "mouse"
glm.pred.coef$predictor <- sub(".z", "", rownames(glm.pred.coef))
glm.pred.coef <- glm.pred.coef[,c(7, 1:6)]

# 5. get stats on model performance
pred <- data.frame(trueVal=prediction.set$circle, pred=glm.pred$fitted.values, rows=as.numeric(row.names(prediction.set)), species="mouse")
