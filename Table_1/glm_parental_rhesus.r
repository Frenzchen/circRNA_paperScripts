...
# Table 1
#
# Reads data table for variables (non-normalised) for GLM of rhesus parental genes
...
# 1. Normalisation
df <- read.table("geneTable_rhesus_glm.txt", sep="\t", header=TRUE, as.is=TRUE)
...
glm.pred.coef$species <- "rhesus"
glm.pred.coef$predictor <- sub(".z", "", rownames(glm.pred.coef))
glm.pred.coef <- glm.pred.coef[,c(7, 1:6)]

# 5. get stats on model performance
pred <- data.frame(trueVal=prediction.set$circle, pred=glm.pred$fitted.values, rows=as.numeric(row.names(prediction.set)), species="rhesus")
