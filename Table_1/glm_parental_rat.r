...
# Table 1
#
# Reads data table for variables (non-normalised) for GLM of rat parental genes
...
# 1. Normalisation
df <- read.table("geneTable_rat_glm.txt", sep="\t", header=TRUE, as.is=TRUE)
#df[,c(2:5,8:12)] <- scale(df[,c(2:5,8:12)])

# 2. Data partitioning
tr <- createDataPartition(df$circle, p = .8, list = FALSE, times = 1)
training.set <- df[tr, c(2:5,8:13)]
nrow(training.set)
prediction.set <- df[-tr, c(2:5,8:13)]
nrow(prediction.set)
...
glm.pred.coef$species <- "rat"
glm.pred.coef$predictor <- sub(".z", "", rownames(glm.pred.coef))
glm.pred.coef <- glm.pred.coef[,c(7, 1:6)]

# 5. get stats on model performance
pred <- data.frame(trueVal=prediction.set$circle, pred=glm.pred$fitted.values, rows=as.numeric(row.names(prediction.set)), species="rat")
